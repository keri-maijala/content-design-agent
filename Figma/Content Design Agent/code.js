...
async function loadAllFonts(figmaNode) {
    const seen = new Set();
    for (let i = 0; i < figmaNode.characters.length; i++) {
        const fn = figmaNode.getRangeFontName(i, i + 1);
        if (fn && 'family' in fn) {
...
function captureStyles(figmaNode) {
...
function applyStyleMap(figmaNode, styleMap, replacements) {
    ...
    while (i < newStyleMap.length && i < newLen) {
        ...
        let j = i + 1;
        while (j < newStyleMap.length && j < newLen && newStyleMap[j] === si)
            j++;
        try {
            if (style.fontName && typeof style.fontName === 'object' && 'family' in style.fontName)
                figmaNode.setRangeFontName(i, j, style.fontName);
        }
        catch (e) { }
        try {
            if (typeof style.fontSize === 'number')
                figmaNode.setRangeFontSize(i, j, style.fontSize);
        }
        catch (e) { }
        try {
            if (Array.isArray(style.fills))
                figmaNode.setRangeFills(i, j, style.fills);
        }
        catch (e) { }
        try {
            if (style.letterSpacing && typeof style.letterSpacing === 'object')
                figmaNode.setRangeLetterSpacing(i, j, style.letterSpacing);
        }
        catch (e) { }
        try {
            if (style.lineHeight && typeof style.lineHeight === 'object')
                figmaNode.setRangeLineHeight(i, j, style.lineHeight);
        }
        catch (e) { }
        try {
            if (style.textDecoration)
                figmaNode.setRangeTextDecoration(i, j, style.textDecoration);
        }
        catch (e) { }
        i = j;
    }
...
function findFragment(text, fragment, hintIndex) {
    const normalize = (s) => s
        .replace(/[\u2018\u2019]/g, "'")
        .replace(/[\u201C\u201D]/g, '"')
        .replace(/[\n\r]/g, ' ');
    // Word boundary check: prevents matching "Home" inside "HomeHub"
    const isWordBoundaryMatch = (t, i, frag) => {
    ...
    if (hintIndex !== undefined && hintIndex !== null && hintIndex >= 0 && hintIndex + fragment.length <= text.length) {
        ...
        for (let delta = -5; delta <= 5; delta++) {
            ...
            if (i < 0 || i + fragment.length > text.length)
                continue;
...
async function applyFixes(fixes) {
    ...
    for (const fix of fixes) {
        ...
        if (!fixesByNode.has(node.id))
            fixesByNode.set(node.id, []);
        fixesByNode.get(node.id).push(fix);
    }
    for (const [nodeId, nodeFixes] of fixesByNode) {
        const figmaNode = figma.getNodeById(nodeId);
